...
class HybridStorageService {
  ...
  determineStorageProvider(fileSize, fileType) {
    // Always use Supabase for small files (free tier)
    if (fileSize <= STORAGE_THRESHOLDS.SMALL) {
      return 'supabase';
    }
    
    // Use R2 for medium files (cost-effective)
    if (fileSize <= STORAGE_THRESHOLDS.MEDIUM) {
      return 'r2';
    }
    
    // Use local storage for large files (free but not globally accessible)
    if (fileSize <= STORAGE_THRESHOLDS.LARGE) {
      return process.env.LOCAL_STORAGE_ENABLED === 'true' ? 'local' : 'r2';
    }
  ...
  async uploadFile(file, options = {}) {
    try {
      ...
      // Upload to appropriate provider, with fallback when Supabase rejects MIME type
      let uploadResult;
      try {
      switch (provider) {
        case 'supabase':
          uploadResult = await this.uploadToSupabase(file, uniqueFilename, fileConfig.bucket, options);
          break;
        case 'r2':
          uploadResult = await this.uploadToR2(file, uniqueFilename, fileConfig.bucket, options);
          break;
        case 'local':
          uploadResult = await this.uploadToLocal(file, uniqueFilename, fileConfig.bucket, options);
          break;
        default:
          throw new Error(`Unsupported storage provider: ${provider}`);
        }
      } catch (provErr) {
        if (provider === 'supabase' && (provErr.statusCode === '415' || provErr?.error === 'invalid_mime_type')) {
          console.warn('⚠️  Supabase rejected mime-type; falling back to R2');
          uploadResult = await this.uploadToR2(file, uniqueFilename, fileConfig.bucket, options);
        } else {
          throw provErr;
        }
  ...
  async getFileUrl(fileId, userId, expiresIn = 3600) {
    try {
      ...
      switch (file.storage_provider) {
        case 'supabase':
          if (file.is_public) {
            ...
            const { data: urlData, error: urlError } = await supabase.storage
              .from(file.bucket)
              .createSignedUrl(file.filename, expiresIn);
            
            if (urlError) throw urlError;
            return { url: urlData.signedUrl, expires: Date.now() + (expiresIn * 1000) };
          }
        ...
        default:
          throw new Error(`Unsupported storage provider: ${file.storage_provider}`);
      }
    } catch (error) {
  ...
  async getStorageStats(userId = null) {
    try {
      ...
      files.forEach(file => {
        ...
        switch (file.storage_provider) {
          case 'supabase':
            stats.supabase_files++;
            stats.supabase_size_mb += sizeMB;
            break;
          case 'r2':
            stats.r2_files++;
            stats.r2_size_mb += sizeMB;
            break;
          case 'local':
            stats.local_files++;
            stats.local_size_mb += sizeMB;
            break;
        }
      });
      ...
      stats.supabase_size_mb = Math.round(stats.supabase_size_mb * 100) / 100;
      stats.r2_size_mb = Math.round(stats.r2_size_mb * 100) / 100;
      stats.local_size_mb = Math.round(stats.local_size_mb * 100) / 100;
